...
interface MatchesErrorPageProps {
  error: Error & { digest?: string };
  reset: () => void;
}

export default function MatchesErrorPage({ error, reset }: MatchesErrorPageProps) {
  return (
    <main className="mx-auto flex min-h-[60vh] w-full max-w-3xl flex-col items-start justify-center gap-4 px-6 py-10 md:px-10">
      ...
      <h1 className="font-display text-[32px] leading-none tracking-[.02em] text-[var(--text)]">No pudimos cargar los partidos</h1>
      <p className="mt-3 text-sm text-[var(--text2)]">
        Ocurrio un error al consultar la informacion. Puedes intentar nuevamente.
      </p>
